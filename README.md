...
This repository is **infrastructure-as-code** for a **bare-metal cluster footprint on Oracle Cloud Infrastructure (OCI)**. It is not a substitute for Kove product documentation; it provisions network, head + worker nodes, and optional automation so you can run **RHEL on OCI bare metal** with **RDMA-oriented** setup.
...
| **RDMA** | **Remote direct memory access** supports **low CPU overhead** and **low latency** between nodes. OCI documents **HPC / RDMA** networking for suitable shapes; this stack’s Ansible path configures **RHEL**-side **RDMA auth** on the BMs. Many default “VPC only” footprints elsewhere **do not** expose the same **RDMA on bare metal** story without different SKUs or constraints. |
| **Cluster networking** | **HPC cluster** networking aims at **predictable, low-contention** bandwidth between nodes. Generic cloud **oversubscribed** virtual networks are often a poor match for **tightly coupled** memory and I/O patterns; OCI’s **HPC-oriented** networking model is aligned with that requirement. |
...
The stack is **OCI-specific** because it relies on **OCI bare metal**, **documented RDMA / HPC networking patterns**, and **compute-cluster style placement**—not because application code cannot run elsewhere, but because **those ingredients together** are the usual prerequisite for this class of deployment.

### What this stack deploys

This **OCI Resource Manager** / Terraform stack builds a small **HPC cluster**:
...
| **Custom RHEL image** | Bare metal nodes boot from a **RHEL** image you **import** (see [Steps](#steps)). Match the stack variable for **BM image OCID**. |
| **SSH public key** | OCI injects this at launch. Use the **public** key in Terraform / the stack wizard; keep **private** keys on secure systems only. |
...
Whoever runs **Plan/Apply** (Resource Manager job owner, or `terraform` on a workstation) needs permission to manage the resources this stack creates—for example:
...
If these are missing, fix head↔BM SSH, then rerun the playbook from head:
...
| Document | Contents |
|----------|----------|
| **This README** | Overview, prerequisites, high-level steps |
...
| **[docs/HEAD-BM-SSH-README.md](docs/HEAD-BM-SSH-README.md)** | Passwordless SSH from head to bare metal nodes |
| **[FAQ.md](FAQ.md)** | Capacity, reservations, RDMA checks, common errors |
